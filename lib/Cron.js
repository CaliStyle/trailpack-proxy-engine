...
const CronProxyHandler = {
  get (target, key) {
    ...
    const unallowedMethods = ['cancelNext', 'reschedule','nextInvocation','freeze','unfreeze']
    if (
      ...
      return function() {
        return setTimeout(function () {
          target.app.log.debug(`Cron ${key} delayed start by ${ target.timeTilStart / 1000} seconds`)
          return target[key]
        }, target.timeTilStart)
      }
      // return target[key]
...
module.exports = class Cron {
  constructor (app) {
    ...
    Object.defineProperty(this, 'scheduler', {
      ...
      // get: app.proxyEngine.scheduler.Job
    })
