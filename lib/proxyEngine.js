...
module.exports = {
  ...
  configure: (app) => {
    ...
    Object.keys(app.crons).forEach(function(key) {
      ...
      if (
        ...
        && app.config.proxyEngine.crons_config.profiles[profile].crons
      ) {
        app.config.proxyEngine.crons_config.profiles[profile].crons.forEach(allowed => {
   ...
   */
  cancelCrons: (app) => {
    for (const j in app.proxyEngine.schedule.scheduledJobs){
      if (app.proxyEngine.schedule.scheduledJobs.hasOwnProperty(j)) {
        const job = app.proxyEngine.schedule.scheduledJobs[j]
        job.cancel()
      }
    }
    return Promise.resolve()
